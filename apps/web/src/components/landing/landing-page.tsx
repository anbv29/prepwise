    ...
    <div className="min-h-screen overflow-x-hidden bg-[var(--paper)]">
      <header className="app-header sticky top-0 z-50 border-b border-[var(--border)]">
        <div className="mx-auto flex min-h-16 max-w-7xl items-center justify-between gap-5 px-5 sm:px-8">
          <Link
            className="flex shrink-0 items-center gap-3 font-semibold tracking-[-0.01em]"
            href="/"
          >
            <BrandMark /> Prepwise
          </Link>
          <nav aria-label="Primary navigation" className="hidden items-center gap-1 lg:flex">
            {navigation.map(([label, href]) => (
              <a
                className="rounded-lg px-3 py-2 text-sm font-medium text-[var(--muted)] hover:bg-[var(--surface-subtle)] hover:text-[var(--ink)]"
                href={href}
                key={href}
              >
                {label}
          ...
          <div className="flex items-center gap-2">
            <details className="relative lg:hidden">
              <summary className="grid h-10 w-10 cursor-pointer list-none place-items-center rounded-lg border border-[var(--border)] text-[var(--muted)] [&::-webkit-details-marker]:hidden">
                <span className="sr-only">Open navigation</span>
                <Menu size={18} />
              </summary>
              <nav
                aria-label="Mobile navigation"
                className="absolute right-0 top-12 grid w-52 rounded-xl border border-[var(--border)] bg-[var(--surface)] p-2 surface-shadow"
              >
                {navigation.map(([label, href]) => (
                  <a
                    className="rounded-lg px-3 py-2 text-sm font-medium hover:bg-[var(--surface-subtle)]"
                    href={href}
                    key={href}
                  >
                    {label}
      ...
      <main>
          ...
          <div className="mx-auto grid min-h-[calc(100vh-4rem)] max-w-7xl items-center gap-14 px-5 py-16 sm:px-8 lg:grid-cols-[minmax(0,1fr)_minmax(420px,0.82fr)] lg:py-20">
            <div className="max-w-3xl">
              ...
              <p className="mt-7 max-w-2xl text-lg leading-8 text-[var(--muted)] sm:text-xl">
                Paste a job description and company URL. Prepwise turns them into grounded company
                context, requirement-linked questions, study cards, and a plan for the days you have
                left.
              </p>
              <div className="mt-9 flex flex-wrap items-center gap-3">
                <Link
                  className="inline-flex min-h-12 items-center gap-2 rounded-xl bg-[var(--accent)] px-6 font-semibold text-white hover:opacity-90"
                  href="/register"
                >
                  Build your first kit <ArrowRight size={18} />
                </Link>
                <a
                  className="inline-flex min-h-12 items-center rounded-xl border border-[var(--border-strong)] px-6 font-semibold hover:border-[var(--accent)] hover:text-[var(--accent)]"
                  href="#how-it-works"
                >
                  See how it works
                </a>
              ...
              <dl className="mt-12 grid max-w-2xl grid-cols-3 border-y border-[var(--border)] py-5">
                <div>
                  <dt className="text-sm text-[var(--muted)]">Analysis stages</dt>
                  <dd className="mt-1 text-xl font-semibold tabular-nums">6</dd>
                </div>
                <div className="border-l border-[var(--border)] pl-5">
                  <dt className="text-sm text-[var(--muted)]">Must-have coverage</dt>
                  <dd className="mt-1 text-xl font-semibold tabular-nums">100%</dd>
                </div>
                <div className="border-l border-[var(--border)] pl-5">
                  <dt className="text-sm text-[var(--muted)]">Plan length</dt>
                  <dd className="mt-1 text-xl font-semibold tabular-nums">1–30 days</dd>
                </div>
              </dl>
            </div>
            ...
            <div className="mx-auto w-full max-w-xl lg:mx-0">
              <div className="border border-[var(--border-strong)] bg-[var(--surface)] p-4 surface-shadow sm:p-5">
                <div className="flex items-center justify-between border-b border-[var(--border)] pb-4">
                  <div>
                    <p className="text-sm font-semibold">Senior Platform Engineer</p>
                    <p className="mt-1 text-sm text-[var(--muted)]">Northstar Labs</p>
                  </div>
                  <span className="rounded-full bg-[var(--success-soft)] px-3 py-1 text-sm font-semibold text-[var(--success)]">
                    Ready
                  </span>
                </div>
                <div className="grid gap-3 py-5 sm:grid-cols-2">
                  {[
                    { title: 'Company brief', detail: '2 verified sources', icon: Building2 },
                    { title: 'Role analysis', detail: '8 mapped requirements', icon: Target },
                    {
                      title: 'Questions',
                      detail: '18 role-specific prompts',
                      icon: MessageSquareText,
                    },
                    { title: 'Coverage', detail: 'All must-haves covered', icon: BadgeCheck },
                  ].map(({ title, detail, icon: Icon }) => (
                    <div
                      className="border-l-2 border-[var(--accent)] bg-[var(--surface-subtle)] px-4 py-3"
                      key={title}
                    >
                ...
                <div className="border-t border-[var(--border)] pt-4">
                  <div className="mb-2 flex items-center justify-between text-sm">
                    <span className="font-semibold">Today’s preparation</span>
                    <span className="text-[var(--muted)] tabular-nums">42 minutes</span>
                  </div>
                  {[
                    'System design and failure recovery',
                    'TypeScript service boundaries',
                    'Cross-functional story review',
                  ].map((item, index) => (
                    <div
                      className="flex items-center gap-3 border-t border-[var(--border)] py-3 first:border-0"
                      key={item}
                    >
                      <span className="grid h-7 w-7 shrink-0 place-items-center rounded-full bg-[var(--accent-soft)] text-xs font-semibold text-[var(--accent)] tabular-nums">
                        {index + 1}
                      </span>
        ...
        <section className="scroll-mt-20" id="about">
          <div className="mx-auto grid max-w-7xl gap-10 px-5 py-20 sm:px-8 lg:grid-cols-[0.62fr_1fr] lg:py-28">
            <div>
              <p className="text-sm font-semibold text-[var(--accent)]">About Prepwise</p>
              <h2 className="mt-3 text-3xl font-semibold tracking-[-0.035em] sm:text-4xl">
                Generic preparation misses the point.
              </h2>
            </div>
            <div className="max-w-3xl space-y-5 text-lg leading-8 text-[var(--muted)]">
              <p>
                Every interview is shaped by a specific company, role, seniority level, and set of
                expectations. A broad list of common questions cannot tell you which topics deserve
                the most time.
              </p>
              <p>
                Prepwise starts from the source material. It separates verified company context from
                weak public signals, maps questions to requirements, checks what remains uncovered,
                and builds a realistic preparation sequence.
              </p>
            </div>
          </div>
        </section>
        ...
        >
          <div className="mx-auto max-w-7xl px-5 py-20 sm:px-8 lg:py-28">
            ...
            <h2 className="mt-3 max-w-2xl text-3xl font-semibold tracking-[-0.035em] sm:text-4xl">
              One source becomes a complete preparation system.
            </h2>
        ...
        <section className="scroll-mt-20" id="how-it-works">
          <div className="mx-auto max-w-7xl px-5 py-20 sm:px-8 lg:py-28">
            ...
            <h2 className="mt-3 max-w-2xl text-3xl font-semibold tracking-[-0.035em] sm:text-4xl">
              From job listing to interview-ready in three steps.
            </h2>
            <ol className="mt-12 grid gap-px border border-[var(--border)] bg-[var(--border)] lg:grid-cols-3">
              {[
                [
                  '01',
                  'Add the role',
                  'Paste the full job description, company URL, and the number of days before your interview.',
                ],
                [
                  '02',
                  'Review the research',
                  'Prepwise analyses the role, checks company sources, and builds linked study material.',
                ],
                [
                  '03',
                  'Follow the plan',
                  'Work through the hardest must-have topics first, then revisit them with scheduled review.',
                ],
              ].map(([number, title, text]) => (
                <li className="bg-[var(--paper)] p-7 sm:p-9" key={number}>
          ...
          <div className="mx-auto max-w-7xl px-5 py-20 sm:px-8 lg:py-28">
            ...
            <div className="mt-12 grid border border-[var(--border)] lg:grid-cols-3">
              {plans.map((plan, index) => (
                <article
                  className={`flex h-full flex-col p-7 sm:p-8 ${index > 0 ? 'border-t border-[var(--border)] lg:border-l lg:border-t-0' : ''} ${plan.featured ? 'bg-[#16161b] text-white' : ''}`}
                  key={plan.name}
                >
                  ...
                  <p className="mt-7">
                    ...
                    {plan.suffix ? (
                      <span className="ml-1 text-sm opacity-70">{plan.suffix}</span>
                    ) : null}
                  </p>
                  <p
                    className={`mt-4 min-h-14 ${plan.featured ? 'text-white/70' : 'text-[var(--muted)]'}`}
                  >
                    {plan.description}
                  </p>
                  <ul className="mt-7 flex-1 space-y-3 pb-9">
                    {plan.features.map((feature) => (
                      <li className="flex gap-3 text-sm" key={feature}>
                        <Check className="mt-0.5 shrink-0 text-[var(--success)]" size={16} />{' '}
                        {feature}
                      </li>
                    ))}
                  </ul>
        ...
        <section className="scroll-mt-20" id="reviews">
          <div className="mx-auto grid max-w-7xl gap-12 px-5 py-20 sm:px-8 lg:grid-cols-[0.48fr_1fr] lg:py-28">
            <div>
              <p className="text-sm font-semibold text-[var(--accent)]">Candidate reviews</p>
              <h2 className="mt-3 text-3xl font-semibold tracking-[-0.035em] sm:text-4xl">
                Useful because it stays specific.
              </h2>
            </div>
            <div className="divide-y divide-[var(--border)] border-y border-[var(--border)]">
              {[
                [
                  '“The coverage view showed exactly which backend requirements I had not prepared an example for.”',
                  'Backend engineering candidate',
                ],
                [
                  '“The schedule stopped me spending three days on easy questions and leaving system design until the night before.”',
                  'Senior software candidate',
                ],
                [
                  '“I could see where the company brief came from, which made it much easier to trust and verify.”',
                  'Product engineering candidate',
                ],
              ].map(([quote, role]) => (
                <figure className="py-7" key={role}>
                  <blockquote className="text-xl font-medium leading-8 tracking-[-0.015em]">
                    {quote}
                  </blockquote>
                  <figcaption className="mt-4 text-sm text-[var(--muted)]">{role}</figcaption>
                </figure>
              ))}
            </div>
          </div>
        ...
        <section
          className="scroll-mt-20 border-t border-[var(--border)] bg-[#16161b] text-white"
          id="contact"
        >
          <div className="mx-auto grid max-w-7xl items-end gap-10 px-5 py-20 sm:px-8 lg:grid-cols-[1fr_auto] lg:py-24">
            <div className="max-w-3xl">
              <p className="text-sm font-semibold text-[#a5b4fc]">Contact</p>
              <h2 className="mt-3 text-3xl font-semibold tracking-[-0.035em] sm:text-5xl">
                Have a question about your preparation workflow?
              </h2>
              <p className="mt-5 max-w-2xl text-white/65">
                Tell us what you are preparing for, what is missing, or where the tool could be
                clearer.
              </p>
            </div>
            <a
              className="inline-flex min-h-12 items-center gap-2 justify-self-start rounded-xl bg-white px-6 font-semibold text-[#16161b]"
              href="mailto:hello@prepwise.app"
            >
              hello@prepwise.app <ChevronRight size={17} />
            </a>
          </div>
        </section>
      </main>

      <footer className="border-t border-white/10 bg-[#16161b] text-white/60">
